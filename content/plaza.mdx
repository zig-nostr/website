...
**Plaza** is a fast, local-first Nostr client for macOS and Linux, built natively in Zig on
...
There is no exception, including in a source build. Pasting a secret key into
the sign-in field does not import it: Plaza refuses it without taking it into
memory, leaves the text in the field you typed it in, and opens the keyholder so
the key goes to the app that holds keys. A test asserts the old wording is not
drawn anywhere, so this cannot quietly come back.
...
The same app, with one thing to know. macOS hands text to the system to draw. On
Linux there is nothing to hand it to, so Plaza draws every glyph from faces it
carries: emoji are drawn in colour, and Latin, Cyrillic and Greek are covered.
Scripts those faces do not carry, CJK, Japanese, Korean, Arabic, Hebrew, Thai and
Devanagari among them, are drawn as empty boxes rather than as text. If you read
Nostr in one of those, the macOS build is the one to use today.
