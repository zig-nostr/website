...
It is the same app, not a port with pieces missing. What differs is underneath:
macOS hands text to the system to draw, and on Linux there is nothing to hand it
to, so Plaza draws every glyph itself and carries its own colour emoji face.
