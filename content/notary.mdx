...
On a reasonably recent distribution: **Ubuntu 23.10+, Debian 13+, or Fedora
39+**. The binaries are built against glibc 2.38 and the toolkit's Linux host
wants GTK 4.10, so Ubuntu 22.04 and Debian 12 are too old; the installer checks
that before it downloads anything. GTK 4 is the one runtime dependency beyond
that, and it says so before it downloads anything too. It verifies the SHA-256
and installs into `~/.local`, so nothing needs root and nothing lands outside
your home directory.
